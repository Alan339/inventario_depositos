...
def monitores():
	...
	root.title("Modificar monitores")
	...
	tablamenu.add_command(label="Monitores",state="disabled")
	...
	def llenar_tabla():
		...
		instruccion="SELECT * FROM monitores"
	...
	def seleccionar(event):
		...
		if int(registro)>0:
			...
			monitor.set(tabla.item(registro, "values")[2])
			...
			recomendaciones.set(tabla.item(registro, "values")[7])

	Label(miFrame, text="TABLAS DE MONITORES", fg="black", font=("Comic Sans MS", 18)).grid(row=0, column=1, columnspan=3)
	...
	cuadroRecomendaciones=Entry(miFrame,textvariable=recomendaciones,width=70)
	cuadroRecomendaciones.grid(row=11, column=1, pady=10,columnspan=3)
	...
	MarcaLabel.grid(row=4, column=1, columnspan=2,pady=10)
	...
	def boton1():
		...
		val=(marca.get(),modelo.get(),check.get(),observaciones.get(),recomendaciones.get())

		instruccion="UPDATE monitores SET MARCA=%s, MODELO=%s, FUNCIONA=%s, OBSERVACIONES=%s, RECOMENDACIONES=%s WHERE ID="+registro
	...
	tabla.heading('#3',text="Monitor",anchor=CENTER)
	...
	tabla.heading('#8',text="Recomendaciones",anchor=CENTER)
