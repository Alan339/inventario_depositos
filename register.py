...
label3=Label(frame,text="Número de curso:").grid(row=3,column=0,sticky="w",pady=5,padx=5)
...
Entrada3=Entry(frame,textvariable=tabla).grid(row=3,column=1,sticky="e",pady=5,padx=5)
Radiobutton(frame,text="Admin",variable=opcion,value=1).grid(row=4,column=0,sticky="nsew",pady=5,padx=5)
Radiobutton(frame,text="Usuario",variable=opcion,value=2).grid(row=4,column=1,sticky="nsew",pady=5,padx=5)
...
def click_boton():
	...
	val=(usuario.get(),p_encriptada.get(),tabla.get(),tipoUsuario.get())
	
	instruccion="INSERT INTO usuarios (USUARIO,CONTRASENA, N_CURSO, TIPO_USUARIO) values (%s,%s,%s,%s)"
...
botonEnviar = Button(frame,text="Enviar",padx=5,pady=5,command=click_boton).grid(row=5,column=0,columnspan=2)
