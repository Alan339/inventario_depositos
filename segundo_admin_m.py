...
def v_segundo():
	...
	def llenar_tabla():
		...
		instruccion="SELECT * FROM segundo"
	...
	def seleccionar(event):
		...
		if int(registro)>0:
			...
			materias_adeudadas.set(tabla.item(registro, "values")[4])
			fecha_ingreso.set(tabla.item(registro, "values")[5])
			observacion.set(tabla.item(registro, "values")[6])

	Label(frame, text="ADMINISTRACIÓN DE ALUMNOS",font=("Calibri",20,"bold")).grid(row=0,column=0,padx=5,pady=2)
	Label(frame, text="Segundo",font=("Calibri",12,"bold")).grid(row=1,column=0,padx=5,pady=2)
	...
	cuadroMaterias=Entry(miFrame,width=70,textvariable=materias_adeudadas)
	cuadroMaterias.grid(row=5, column=0,columnspan=3, pady=10)
	cuadroMaterias.config(justify="center")
	...
	RecomendacionesLabel=Label(miFrame, text="Materias Adeudadas:")
	RecomendacionesLabel.grid(row=4, column=0, columnspan=3)
	...
	def boton2():
		...
		segundo_admin.v_segundo()

	def boton3():

		registro=tabla.selection()[0]

		val=(nombres.get(),apellidos.get(),dni.get(),materias_adeudadas.get(),fecha_ingreso.get(),observacion.get())

		instruccion="UPDATE segundo SET NOMBRES=%s, APELLIDOS=%s, DNI=%s, MATERIAS_ADEUDADAS=%s, FECHA_INGRESO=%s, OBSERVACION=%s WHERE ID="+registro
