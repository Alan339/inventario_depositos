...
def v_sexto():
	...
	Label(frame, text="Sexto",font=("Calibri",12,"bold")).grid(row=1,column=0,padx=5,pady=2)
	...
	def boton3():
		...
		instruccion="UPDATE sexto SET NOMBRES=%s, APELLIDOS=%s, DNI=%s, MATERIAS_ADEUDADAS=%s, FECHA_INGRESO=%s, OBSERVACION=%s WHERE ID="+registro
	...
	tabla_frame=LabelFrame(root,text="Curso de sexto:")
	tabla_frame.pack()

	scroll = Scrollbar(tabla_frame)
	scroll.pack(side=RIGHT,fill=Y)

	tabla=ttk.Treeview(tabla_frame,yscrollcommand=scroll.set,height=10,show="headings",columns=('#1','#2','#3','#4','#5','#6','#7'))
